(function(){
  ...
  // cart helpers
  function getCart(){try{return JSON.parse(localStorage.getItem("forgewear_cart")||"[]")}catch(e){return[]}}
  function setCart(c){localStorage.setItem("forgewear_cart",JSON.stringify(c));updateCartCount()}
  ...
  if(checkoutForm)checkoutForm.addEventListener("submit",function(e){
    e.preventDefault();
    localStorage.removeItem("forgewear_cart");
    updateCartCount();
